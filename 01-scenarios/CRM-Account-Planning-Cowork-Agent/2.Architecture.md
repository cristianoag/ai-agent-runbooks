...
## Choosing the CRM Integration

These ways to get CRM data into a Copilot experience are not interchangeable.
...
| **Copilot connector (federated MCP)** | Live, read-only external retrieval in supported Microsoft 365 Copilot experiences, without an external-data index | Provider/tenant and authentication fit must be validated. Not write-back, complete-dataset analytics, or a general custom-agent tool connection |
...
### Distinguish the federated retrieval option

[MCP Federated Connectors](../../02-patterns/MCP-Federated-Connectors/MCP-Federated-Connectors.md)
adds a read-only external-retrieval option in supported Microsoft 365 Copilot Cowork experiences.
It is not the custom MCP plugin depicted above. Use it for current account facts and supporting
documents only after confirming source/provider availability, custom-object coverage, and seller
record permissions; Salesforce is not assumed to be a ready-made gallery connection.

An organization-built connector needs a compatible MCP server and supported authentication,
validated end-to-end with the [federated connector runbook](../../02-patterns/MCP-Federated-Connectors/MCP-Federated-Connectors-Runbook.md).
Do not treat retrieved search results as a complete pipeline dataset. Retain deterministic
server-side queries for complete aggregates and the existing plugin/action path for write-back.
Federation itself does not provide scheduled orchestration, Office artifact generation, or
approval gates.
