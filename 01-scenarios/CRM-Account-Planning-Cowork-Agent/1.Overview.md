...
**Read-only account research option:** [MCP Federated Connectors](../../02-patterns/MCP-Federated-Connectors/MCP-Federated-Connectors.md)
can provide live external account context in supported Copilot Cowork experiences without a
Microsoft 365 external-data index. Qualify the provider, tenant, custom-field coverage, and
per-user access; a Salesforce integration may require an organization-built connector.
Federated retrieval is distinct from this scenario's custom MCP plugin: retain the plugin
and orchestration for CRM write-back, complete server-side pipeline queries, and scheduled
workflows, and retain the existing artifact-generation and approval design.
