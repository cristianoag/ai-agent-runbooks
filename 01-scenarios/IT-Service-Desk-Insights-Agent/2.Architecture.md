...
## Optional Live Knowledge Retrieval

For fresh KB articles or catalog descriptions, a supported Microsoft 365 Copilot experience can
use [MCP Federated Connectors](../../02-patterns/MCP-Federated-Connectors/MCP-Federated-Connectors.md)
as a read-only companion to this indexed declarative agent. Do not substitute a federated connector
into the diagram or assume that Agent Builder can consume it. ServiceNow availability and the
authentication fit must be qualified; a custom connector may be required.

Validate the MCP path against the source's user criteria, including restricted HR articles and
access revocation. The synced connector's ACL mapping above does not prove the live path is safe.
Use the [federated connector runbook](../../02-patterns/MCP-Federated-Connectors/MCP-Federated-Connectors-Runbook.md)
for host and permission tests. Retrieved examples are not an exhaustive ticket population:
retain BI for counts, MTTR, and SLA reporting, and keep incident writes separate.
