...
**Optional live knowledge companion:** [MCP Federated Connectors](../../02-patterns/MCP-Federated-Connectors/MCP-Federated-Connectors.md)
can support read-only lookup of current HR policies and IT guidance in a supported Microsoft 365
Copilot experience, such as Copilot Chat, with source-system user permissions and no Microsoft 365
external-data index. Qualify the source/provider and tenant first; ServiceNow or Workday may need
an organization-built MCP connector. This is not automatic ESS-template or custom-agent support
and does not replace request submission, HR transactions, or the existing synced architecture.
