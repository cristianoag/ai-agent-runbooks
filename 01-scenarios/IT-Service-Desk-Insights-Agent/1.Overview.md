...
fits current KB article and catalog-description lookup in a supported experience such as
Microsoft 365 Copilot Chat, subject to source/provider and tenant support. A ServiceNow source
may require a custom MCP connector with validated user permissions. This does not change the
declarative agent's synced grounding above: federation is not automatically available to that
agent, does not submit catalog requests, and does not supply exhaustive ticket analytics.
