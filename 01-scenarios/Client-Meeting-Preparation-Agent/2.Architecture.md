...
### Federated retrieval qualification

[MCP Federated Connectors](../../02-patterns/MCP-Federated-Connectors/MCP-Federated-Connectors.md)
is a possible read-only research companion in supported Copilot Chat or Researcher experiences,
not a drop-in replacement for the agent's MCP integration above. Qualify the book-of-record
connector, user identity, citations, timestamps, and cross-segment denial tests using the
[federated connector runbook](../../02-patterns/MCP-Federated-Connectors/MCP-Federated-Connectors-Runbook.md).
Custom authentication or multi-hop identity delegation can require server/gateway engineering
and proof in the exact Copilot host, not just wizard configuration.

A private-only MCP endpoint is not reachable merely because a VNet and site-to-site VPN exist.
Federation has no documented private-network attachment or Graph connector-agent relay. Require
an approved public authenticated gateway forwarding privately, or retain a different architecture.
No external-data index does not remove the need to approve the processing of retrieved client data.

### Existing information-barrier requirements
