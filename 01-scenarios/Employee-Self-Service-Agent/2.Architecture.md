...
### Optional companion: live external knowledge

The diagram and build options above remain the primary ESS architecture. Where policy freshness
requires query-time retrieval, evaluate [MCP Federated Connectors](../../02-patterns/MCP-Federated-Connectors/MCP-Federated-Connectors.md)
in a supported Microsoft 365 Copilot experience, not as an assumed ESS or Copilot Studio capability.
Federation retrieves read-only source content without creating a Microsoft 365 external-data index.

Validate source-user authorization for country-scoped HR content, denied access, and permission
changes. Gallery availability is provider/tenant specific; HRIS and ITSM sources may need a custom
MCP server and a supported authentication design. The
[federated connector runbook](../../02-patterns/MCP-Federated-Connectors/MCP-Federated-Connectors-Runbook.md)
provides the qualification path. Keep portal links and existing action integrations for leave
requests, payroll operations, and ticket submission; review processing and retention of retrieved
HR content even though it is not indexed.

### Existing ESS integrations
